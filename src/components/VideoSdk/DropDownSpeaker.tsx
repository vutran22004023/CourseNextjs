...
import DropSpeaker from "@/assets/iconSvg/DropSpeaker";
import TestSpeaker from "@/assets/iconSvg/TestSpeaker";
import { useMeetingAppContext } from "@/context/MeetingAppProvider";

import useSound from "use-sound";

export default function DropDownSpeaker({ speakers }: any) {
  const { setSelectedSpeaker, selectedSpeaker, isMicrophonePermissionAllowed } =
    useMeetingAppContext();
  const [audioProgress, setAudioProgress] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [isHovered, setIsHovered] = useState(false);
  const [notificationSound] = useSound("/sounds/test_sound.mp3");
  const testSpeakers = () => {
                  ...
                  <div className={"bg-gray-350 rounded-lg"}>
                    <div>
                      <div className="flex flex-col">
                        {speakers.map((item: any, index: any) => {
                          return (
                        ...
                        {speakers.length && (
                          <>
                            ...
                            <div
                              className={`my-1 pl-4 pr-2 text-white text-left`}
                            >
                              <button
