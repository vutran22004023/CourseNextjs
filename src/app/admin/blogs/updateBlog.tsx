...
const UpdateBlog: React.FC<UpdateProps> = ({ data, isOpen, onClose }) => {
  ...
  const mutationUpdate = useMutationHook(async (dataForm: any) => {
    ...
    const res = await UpdateBlog(data._id, dataForm, accessToken);
    return res;
  });
        ...
        <div className="max-h-[580px] overflow-y-auto">
          <FormProvider {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                <Label>Slug</Label>
                <Input placeholder="Nhập slug (tự động tạo từ tiêu đề nếu để trống)" {...form.register('slug')} />
              </div>
              ...
              <div>
                ...
                <Input placeholder="Nhập ngày đăng và thời gian đọc (Ví dụ: '5 ngày trước - 3 phút đọc')" {...form.register('date')} />
              </div>

              <ButtonComponent type="submit">Cập nhật bài viết</ButtonComponent>
            </form>
          </FormProvider>
        </div>
        <SheetFooter>
          <SheetClose asChild>
            <ButtonComponent type="button">Đóng</ButtonComponent>
          </SheetClose>
        </SheetFooter>
      </SheetContent>
    </Sheet>
