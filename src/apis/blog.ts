...
export const GetAllBlogs = async(search: string) => {
    try{
      if(search){ 
        const response: AxiosResponse = await axios.get(`/api/blog/all-posts?filter=name:${search}`);
...
export const GetDetailBlogs = async(slug: StringConstructor) => {
  try{
      const response: AxiosResponse = await axios.get(`/api/blog/detail-post/${slug}`);
      return response.data;
  }catch {
...
export const CreateBlogs = async (data: any, access_Token: any) => {
    try {
      const response: AxiosResponse = await axios.post(`/api/blog/create-post`, data,{
        headers: {
  ...
  export const UpdateBlogs = async (id: any,data: any, access_Token: any) => {
    try {
      const response: AxiosResponse = await axios.put(`/api/blog/update-post/${id}`, data,{
        headers: {
            token: `Bearer ${access_Token}`,
        }
    });
  ...
  export const DeleteBlogs = async (id: any, access_Token: any) => {
    try {
      const response: AxiosResponse = await axios.delete(`/api/blog/delete-post/${id}`,{
        headers: {
            token: `Bearer ${access_Token}`,
        }
    });
